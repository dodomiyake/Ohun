...
import { MemoryPasswordBlocklist, hashPassword, isExpired, matchesTokenHash, normalizeDeviceName, opaqueToken, signAccessToken, tokenHash, validatePassword, verificationCode, verifyAccessToken, verifyPassword } from './security.js';
...
describe('M2 security primitives', () => {
  ...
  it('compares keyed token hashes safely', () => { const digest = tokenHash(secret, 'verifyCode', '123456'); expect(matchesTokenHash(secret, 'verifyCode', '123456', digest)).toBe(true); expect(matchesTokenHash(secret, 'verifyCode', '654321', digest)).toBe(false); });
